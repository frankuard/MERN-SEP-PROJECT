...
const DashboardShell = ({ title, description }) => {
  const { user } = useAuth();
      ...
      <main className="flex-1 overflow-y-auto px-6 py-8 sm:px-8">
        <div className="mx-auto max-w-5xl">
          ...
          >
            ...
            <p className="mt-1 text-sm capitalize" style={{ color: t.activeText }}>
              {user?.role} account
            </p>
          </div>
        </div>
...
const StudentDashboard = () => (
  <DashboardShell
    ...
    description="Access lost & found, campus help, borrow/lend, posts, and other student services from the sidebar."
  />
);
