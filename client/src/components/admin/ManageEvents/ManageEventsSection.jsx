...
const ManageEventsSection = ({ t, devcorpsMode }) => {
    ...
    // Opens the themed confirmation modal instead of deleting immediately
    const handleDeleteClick = (event) => {
        setDeleteTarget(event);
    };

    // Fires only after the user actually clicks "Delete Event" inside the modal
    const handleConfirmDelete = async () => {
        if (!deleteTarget) return;
        setDeleting(true);
        try {
            await eventsApi.deleteEvent(deleteTarget._id);
        ...
        <div className="space-y-6 animate-in fade-in duration-200">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <button
                    type="button"
                    onClick={openCreate}
                    className="flex items-center gap-1.5 rounded-xl bg-black px-4 py-2 text-xs font-bold text-white transition-opacity hover:opacity-90 sm:text-sm"
                >
                    <Plus size={14} />
                    Add Event
                </button>
            ...
            {deleteTarget && (
                <ConfirmDeleteModal
                    ...
                    message="This also removes all registrations for it. This action cannot be undone."
